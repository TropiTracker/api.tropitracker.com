...
functions.getHurricanes = async function() {
    return await getStormsFiltered('HU');
}

functions.getTropicalStorms = async function() {
    return await getStormsFiltered('TS');
}

functions.getTropicalDepressions = async function() {
    return await getStormsFiltered('TD');
}

functions.getPotentialCyclones = async function() {
    return await getStormsFiltered('PTC');
}

async function getStormsFiltered(filterString) {
    const response = (await axios.get(currentStormsUrl)).data.activeStorms;
    const filteredResponse = response.filter(storm => storm.classification == filterString);
